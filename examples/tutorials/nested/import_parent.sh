...
db="palm_static"
schema="inputs_tutorial_nested_parent"
owner=palm
base_path=./parent/
...
#
## raster layers
rshapes=('dem.tif' 'buildings.tif')
rlayers=('dem' 'buildings')
...
for i in ${!rlayers[*]}; do
  ...
  /usr/pgsql-17/bin/raster2pgsql -I -C -M -t auto $base_path/${rshapes[i]} -q $schema.${rlayers[i]} | psql -q -b -d $db
